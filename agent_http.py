...
Usage:
    python agent_http.py              # runs on :8080
    PORT=9000 python agent_http.py    # custom port
...
class BriefHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/health":
            self._respond(200, {"status": "ok", "model": "gemini-2.0-flash"})
        else:
            self._respond(404, {"error": "Not found"})
...
if __name__ == "__main__":
    ...
    print(f"✓ Brief Generator running on port {port}  (Gemini 2.0 Flash)")
